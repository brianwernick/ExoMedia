...
class CaptionPopupManager {
    ...
    fun getCaptionItems(videoView: VideoView): List<CaptionItem> {
        ...
        for (groupIndex in 0 until trackGroupArray.length) {
            val selectedIndex =
                videoView.getSelectedTrackIndex(RendererType.CLOSED_CAPTION, groupIndex)
        ...
        items.add(
            ...
            CaptionItem(
                CC_DEFAULT,
                videoView.context.getString(R.string.auto),
                rendererEnabled && !trackSelected
            )
        )
        items.add(
            0,
            CaptionItem(
                CC_DISABLED,
                videoView.context.getString(R.string.disable),
                !rendererEnabled
            )
        )
