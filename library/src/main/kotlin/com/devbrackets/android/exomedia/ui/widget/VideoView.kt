...
open class VideoView : RelativeLayout {
    ...
    private var overriddenDuration: Long = -1

    private var overridePosition = false
    private var matchOverridePositionSpeed = true

    private var overriddenPositionStopWatch = StopWatch()

    private var muxNotifier = MuxNotifier()
    private val listenerMux: ListenerMux by lazy { ListenerMux(muxNotifier) }
    ...
    fun setMeasureBasedOnAspectRatioEnabled(measureBasedOnAspectRatioEnabled: Boolean) {
        (surface as VideoSurface).setMeasureBasedOnAspectRatioEnabled(
     ...
     */
    private fun initView(context: Context, attributes: VideoViewAttributes) {
        View.inflate(context, R.layout.exomedia_video_view_layout, this)

        // Inflates the correct surface
        findViewById<ViewStub>(R.id.video_view_surface_stub).apply {
            layoutResource =
                if (attributes.useTextureViewBacking) R.layout.exomedia_resizing_texture_view else R.layout.exomedia_resizing_surface_view
            inflate()
        }
     ...
     * @param attributes The attributes associated with this instance
     */
    private fun postInit(attributes: VideoViewAttributes) {
        if (attributes.useDefaultControls) {
            videoControls =
                if (context.isDeviceTV()) VideoControlsLeanback(context) else VideoControlsMobile(
                    context
                )
        }
     ...
     */
    private fun getApiImplementation(): VideoPlayerApi {
     ...
     */
    private fun stopPlayback(clearSurface: Boolean) {
        audioFocusHelper.abandonFocus()
     ...
     * when enabled.
     */
    protected open inner class AudioFocusHelper : AudioManager.OnAudioFocusChangeListener {
     ...
     */
    protected open inner class TouchListener(context: Context) :
