 ...
 */
open class ResizingTextureView : TextureView, VideoSurface {
    private var onSizeChangeListener: OnSizeChangeListener? = null
    ...
    private val attachedListener: OnAttachStateChangeListener = AttachedListener()
    protected var globalLayoutMatrixListener: ViewTreeObserver.OnGlobalLayoutListener =
        GlobalLayoutMatrixListener()
    ...
    private var measureBasedOnAspectRatio: Boolean = false

    /**
     * The scaling method to use for the video
     */
    ...
    override fun clearSurface() {
        ...
        try {
            ...
            gl10.eglChooseConfig(
                display,
                GL_CLEAR_CONFIG_ATTRIBUTES,
                configs,
                configs.size,
                IntArray(1)
            )
            val context = gl10.eglCreateContext(
                display,
                configs[0],
                EGL10.EGL_NO_CONTEXT,
                GL_CLEAR_CONTEXT_ATTRIBUTES
            )
            val eglSurface = gl10.eglCreateWindowSurface(
                display,
                configs[0],
                surfaceTexture,
    ...
    override fun setVideoRotation(@IntRange(from = 0, to = 359) rotation: Int, fromUser: Boolean) {
        setVideoRotation(
            if (fromUser) rotation else requestedUserRotation,
            if (!fromUser) rotation else requestedConfigurationRotation
        )
    }
     ...
     *
     * TODO Should this be owned by the VideoSurfaces instead?
     *
     ...
     * applied incorrectly.
     */
    private fun updateMatrixOnLayout() {
     ...
     */
    private fun notifyOnSizeChangeListener(width: Int, height: Int) {
    ...
    /**
     * This is separated from the [ResizingTextureView.onAttachedToWindow]
    ...
    companion object {
        private const val TAG = "ResizingTextureView"
        protected const val MAX_DEGREES = 360
         ...
         * that context
         */
        private val GL_CLEAR_CONTEXT_ATTRIBUTES =
