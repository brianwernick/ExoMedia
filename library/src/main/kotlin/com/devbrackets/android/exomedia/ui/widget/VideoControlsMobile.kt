...
class VideoControlsMobile : DefaultVideoControls {
  ...
  override fun animateVisibility(toVisible: Boolean) {
    ...
    if (!hideEmptyTextContainer || !isTextContainerEmpty) {
      textContainer.startAnimation(TopViewHideShowAnimation(textContainer, toVisible, DefaultVideoControls.CONTROL_VISIBILITY_ANIMATION_LENGTH))
    }

    if (!isLoading) {
      controlsContainer.startAnimation(BottomViewHideShowAnimation(controlsContainer, toVisible, DefaultVideoControls.CONTROL_VISIBILITY_ANIMATION_LENGTH))
    }
  ...
  override fun updateTextContainerVisibility() {
    ...
    if (hideEmptyTextContainer && emptyText && textContainer.visibility == View.VISIBLE) {
      ...
      textContainer.startAnimation(TopViewHideShowAnimation(textContainer, false, DefaultVideoControls.CONTROL_VISIBILITY_ANIMATION_LENGTH))
    } else if ((!hideEmptyTextContainer || !emptyText) && textContainer.visibility != View.VISIBLE) {
      textContainer.clearAnimation()
      textContainer.startAnimation(TopViewHideShowAnimation(textContainer, true, DefaultVideoControls.CONTROL_VISIBILITY_ANIMATION_LENGTH))
    }
  }
